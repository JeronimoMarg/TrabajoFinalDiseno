...
public class GestorPoliza {
   ...
   public List<Cuota> crearCuotas(PolizaDTO poliza) {
      ...
      if (poliza.getTipoPago() == TipoPago.MENSUAL) {
         for (int i = 0; i < 6; i++) {
            Cuota cuota = new Cuota(poliza.getMonto_a_pagar() / 6.0, poliza.getTipoPago());
            // calcular la fecha de vencimiento de cada cuota!!!
            cuota.setFecha_vencimiento(null);
            cuotas.add(cuota);
         }
      } else {
         Cuota cuota = new Cuota(poliza.getMonto_a_pagar(), poliza.getTipoPago());
         cuotas.add(cuota);
      }
